...
describe('POST /api/v1/auth/login', () => {
  it('should respond with json containing a success message', async () => {
    const response = await request(app).post('/api/v1/auth/login').send({ email: 'superadmin@mail.com', password: 'superadmin1234' }).set('Accept', 'application/json');
  ...
  it('should respond with json containing an error message for invalid login', async () => {
    const response = await request(app).post('/api/v1/auth/login').send({ email: 'wronguser', password: 'wrongpassword' }).set('Accept', 'application/json');
